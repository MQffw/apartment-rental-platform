...
public class LeaseAgreementController {
    ...
    public Result<List<AgreementItemVo>> listItem() {
        ...
        List<AgreementItemVo> list = service.listItemByEmail(email);
    ...
    public Result<AgreementDetailVo> getDetailById(@RequestParam Long id) {
        ...
        AgreementDetailVo agreementDetailVo = service.getDetailById(id, loginUser.getUserId());
        if (agreementDetailVo == null) {
            throw new LeaseException(ResultCodeEnum.DATA_ERROR, "租约记录不存在");
        }
    ...
    public Result saveOrUpdate(@RequestBody LeaseAgreement leaseAgreement) {
        ...
        // 用当前登录用户的邮箱填充承租人信息
        leaseAgreement.setEmail(loginUser.getUsername());
