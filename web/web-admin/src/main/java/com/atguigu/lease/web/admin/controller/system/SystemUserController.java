...
public class SystemUserController {
    ...
    public Result<Boolean> saveOrUpdate(@RequestBody SystemUser systemUser) {
        String password = systemUser.getPassword();
        // 判断是否为有效的密码（需要加密）
        boolean needEncrypt = password != null &&
                !password.trim().isEmpty() &&
                !"null".equalsIgnoreCase(password.trim()) &&
        ...
        boolean success = service.saveOrUpdate(systemUser);
        return success ? Result.ok(true) : Result.fail(ResultCodeEnum.SERVICE_ERROR.getCode(), "保存用户失败");
    }
